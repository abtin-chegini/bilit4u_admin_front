...
const DestinationSearchBox: React.FC = () => {
  // Get everything we need from the Destination store
  const {
    query,
    setQuery,
    value,
  ...
  useEffect(() => {
    // Check if we have a valid destination city in the store
    if (isDestinationValid() && destinationCity) {
      setValue(destinationCity);
      // console.log("Using valid destination city from store:", destinationCity);
    } else {
      // If store doesn't have valid data, try localStorage (for backward compatibility)
      const storedCityName = localStorage.getItem("destinationCityName");
      const storedCityId = localStorage.getItem("destinationCityId");
      ...
      if (storedCityName && storedCityId) {
        ...
        setDestinationCity(storedCityId, storedCityName);
      } else {
        // No valid destination city found anywhere
        setValue("شهر مقصد");
      }
    }
    ...
    const checkInterval = setInterval(() => {
      const valid = isDestinationValid();
      if (!valid && destinationCity) {
        // If destination city expired, update the UI
        setValue("شهر مقصد");
        console.log("Destination city expired, UI refreshed");
      }
    }, 10000); // Check every 10 seconds

    return () => clearInterval(checkInterval);
  }, [setValue, setDestinationCity, destinationCity, isDestinationValid]);
  ...
  useEffect(() => {
    ...
    persianConn.on("ReceiveSearchResultsFA", (results) => {
      // console.log(`📥 Received ${results.length} Persian search results:`, results);
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      if (language === "Persian" && persianConnection) {
        // console.log(`📤 Sending Persian search request for: "${query}"`);

        await persianConnection.invoke("SearchFA", query);
      } else if (language === "English" && englishConnection) {
        // console.log(`📤 Sending English search request for: "${query}"`);
  ...
  const handleSelect = (currentValue: string, cityID: string) => {
    ...
    // Use our store to set the destination city (this also updates localStorage)
    setDestinationCity(cityID, currentValue);

    setOpen(false);
    // console.log("Selected destination city:", { name: currentValue, id: cityID });
  };
  ...
  const handleSelectForPremade = (
    ...
    // Use our store to set the destination city
    setDestinationCity(cityID, currentValue);

    setIsPreMadeSelection(isPreMade);
    setOpen(false);
    // console.log("Selected pre-made destination city:", { name: currentValue, id: cityID });
  };
